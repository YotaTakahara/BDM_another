...
def main():
    ...
    while True:
        ...
        cv.putText(debug_image,"now"+global_string_ans+"global_index_ans"+str(global_index_ans)+"\n"+"global_middle_ans"+str(global_middle_ans)+"\n"+"global_ring_ans"+str(global_ring_ans), (10, 30),
...
def draw_landmarks(image, cx, cy, landmarks, handedness):
    ...
    if len(landmark_point) > 0:
        index_finger= check_finger(landmark_point[5],landmark_point[6],landmark_point[7],landmark_point[8])
        middle_finger= check_finger(landmark_point[9],landmark_point[10],landmark_point[11],landmark_point[12])
        ring_finger= check_finger(landmark_point[13],landmark_point[14],landmark_point[15],landmark_point[16])
        pinky_finger=check_finger(landmark_point[17],landmark_point[18],landmark_point[19],landmark_point[20])
        global global_index_ans
        global global_middle_ans
        global global_ring_ans
        if index_finger==True and middle_finger==False and ring_finger==False:
            global_index_ans+=1
            global_middle_ans=0
            global_ring_ans=0

        elif index_finger==True and middle_finger==True and ring_finger==False:
            global_index_ans=0
            global_middle_ans+=1
            global_ring_ans=0

        elif index_finger==True and middle_finger==True and ring_finger==True and pinky_finger==False:
            global_index_ans=0
            global_middle_ans=0
            global_ring_ans+=1
    ...
    return image
def calc_vector(landmark0,landmark1):
    tmp=[]
    tx=landmark1[0]-landmark0[0]
    ty=landmark1[1]-landmark0[1]
    abs=np.sqrt(tx**2+ty**2)
...
def check_finger(landmark0,landmark1,landmark2,landmark3):
    # tm=np.array(landmark1[0]-landmark0[0],landmark1[1]-landmark0[1])
    # tmp=np.linalg.norm(tm)
    # tmp1=np.linalg.norm(landmark3-landmark0)
