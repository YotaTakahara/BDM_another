...
def main():
    ...
    while True:
        ...
        if enable_segmentation and results.segmentation_mask is not None:
            ...
            fps_color = (0, 255, 0)
        cv.putText(debug_image,global_string+"globl_th_count"+str(global_th_count)+ "global_sum:" + str(global_sum), (10, 30),
...
def draw_landmarks(
    ...
    check_position(landmark_point)                
    return image


def check_position(landmark_point):
    global last_position
    global global_start
    global global_sum
    global global_th
    if global_start==0:
        last_position=landmark_point
        global_start=1
    sum=0
    
    for i in range(len(landmark_point)):
        sum+=np.sqrt((landmark_point[i][1][0]-last_position[i][1][0])**2+(landmark_point[i][1][1]-last_position[i][1][1])**2)
    global_sum=sum
...
def alive_check_alert():
    global global_string
    global global_th_count
    global global_sum
    global global_th
